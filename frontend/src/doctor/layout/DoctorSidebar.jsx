...
const NAV_ITEMS = [
  { to: "/doctor", label: "Overview", icon: <OverviewIcon />, end: true },
  { to: "/doctor/patients", label: "My Patients", icon: <PatientsIcon /> },
  { to: "/doctor/tasks", label: "Tasks", icon: <TasksIcon /> },
  { to: "/doctor/appointments", label: "Appointments", icon: <AppointmentsIcon /> },
];
      ...
      <div className="px-2 py-3 border-t border-white/10">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 text-sm font-semibold
            text-red-300 hover:bg-red-900/30 hover:text-red-200 rounded-lg transition-colors group relative"
        >
          <LogoutIcon />
          {!collapsed && <span>Logout</span>}
          {collapsed && (
            <span className="absolute left-full ml-3 px-2 py-1 rounded-md bg-[#1a2b4a] border border-white/10
              text-xs text-red-300 whitespace-nowrap opacity-0 pointer-events-none
              group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-200 z-50
              shadow-lg">
              Logout
            </span>
