...
const CreateNurseModal = ({ open, onClose }) => {
  const { createNurse, fetchNurses, doctors, fetchDoctors } = useAdminStore();

  const [form, setForm] = useState({ name: "", doctorId: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await createNurse(form);
      setCredentials(data.credentials);
      toast.success("Nurse created successfully");
      await fetchNurses();
    } catch (err) {
      const msg = err?.response?.data?.message || err?.message || "Failed to create nurse";
      setError(msg);
      toast.error(msg);
    } finally {
      ...
      <div className="relative bg-white border border-gray-200 shadow-lg rounded-sm w-full max-w-md mx-4">
        ...
        <div className="bg-[#1a2b4a] text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 bg-emerald-500 rounded-sm flex items-center justify-center text-xs font-bold">
              +
            </div>
            <div>
              <div className="text-sm font-bold tracking-wide">CREATE NURSE</div>
              <div className="text-xs text-blue-300">Assign a nurse to a doctor</div>
            </div>
          </div>
          <button onClick={handleClose} className="text-gray-400 hover:text-white text-lg leading-none">
            ✕
          </button>
        </div>
            ...
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-sm p-4">
                ...
                <p className="text-xs text-green-600 mb-3">
                  Share these credentials with the nurse. The password cannot be recovered later.
                </p>
                ...
                >
                  ...
                  {doctors.map((doc) => (
                    <option key={doc._id} value={doc._id}>
                      {doc.userId?.name} — {doc.department}
                    </option>
                  ))}
                </select>
              </div>
